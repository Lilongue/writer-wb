...
const getNextVersion = (
  ...
  partToBump: 'major' | 'minor' | 'patch',
): string => {
  const parts = currentVersion.split('.').map(Number);
  if (parts.length !== 3 || parts.some(Number.isNaN)) {
    throw new Error(
      `Invalid version format: "${currentVersion}". Expected "major.minor.patch".`,
    );
  }
...
const run = () => {
  try {
    ...
    console.log(
      `Bumping "${partToBump}" version from ${currentVersion} to ${nextVersion}`,
    );
    ...
    filesToUpdate.forEach((filePath) => {
      ...
      console.log(
        `- Updated ${path.relative(rootPath, filePath)} to ${nextVersion}`,
      );
    });
