...
* `TemplateService.ts`: **Менеджер шаблонов объектов мира.**
  * **Ответственность:** Бизнес-логика, связанная с управлением шаблонами объектов мира (CRUD, архивация, переименование), управлением схемой их полей, а также импортом предустановленных шаблонов из файла `assets/predefined-templates.json`.
...
* **Renderer-процесс** (React-компоненты) использует `window.electron.ipcRenderer` для двух типов операций:
  * **Запрос-ответ:** `invoke('channel')` для получения данных от `main`-процесса (например, `get-narrative-items`, `templates:updateSchema`).
