...
* `components/`: Директория для переиспользуемых React-компонентов.
  ...
  * `ContentDisplay.tsx`: Компонент центральной панели, который отображает детальную информацию о выбранном элементе. Для **объектов мира** это включает редактируемые кастомные поля. Для **элементов повествования** это включает редактируемые поля "Основная мысль" и "План", а также содержимое связанного `.md`-файла. Для обеспечения синхронизации с внешними редакторами, компонент использует механизм периодического опроса (polling), проверяя дату модификации открытого файла через IPC.
  * `TemplateManagerModal.tsx`: Модальное окно, предоставляющее интерфейс для управления шаблонами объектов мира. Позволяет создавать, переименовывать, копировать и архивировать шаблоны, а также редактировать схему их пользовательских полей (добавлять, удалять и комментировать поля).
  * `ChecklistEditor.tsx`: Интерактивный компонент для управления чек-листом. Позволяет добавлять, удалять и отмечать пункты. Хранит свое состояние в виде Markdown-строки.
